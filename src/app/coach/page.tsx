...
export default function CoachPage() {
  ...
  useEffect(() => bottomRef.current?.scrollIntoView({ behavior: "smooth" }), [coachMessages, loading]);
  ...
  return (
    <main className="product-page">
      <header className="grid gap-6 border-b border-line pb-8 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          <p className="page-kicker"><BrainCircuit className="h-3.5 w-3.5" /> Mülakat ve kariyer koçu</p>
          <h1 className="page-title-compact mt-4">CV&apos;nizle konuşan bir prova odası.</h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-ink-2">Sorular, son rolünüzden, görünür becerilerinizden ve özgeçmişteki kanıt boşluklarından hazırlanır.</p>
        </div>
        <Button variant="ghost" size="sm" onClick={clearCoach}><RotateCcw className="h-3.5 w-3.5" /> Oturumu temizle</Button>
      </header>
      ...
      <div className="mt-8 grid overflow-hidden border border-line bg-surface xl:min-h-[42rem] xl:grid-cols-[17rem_minmax(0,1fr)_18rem]">
        <aside className="order-2 border-t border-line bg-surface-2 p-5 xl:order-1 xl:border-r xl:border-t-0">
          <p className="section-label">Hazır çalışma notları</p>
          <div className="mt-4 flex flex-wrap gap-1.5">
            <button type="button" onClick={() => setFilter("all")} className={cn("rounded-full px-2.5 py-1 text-[0.6875rem] font-medium", filter === "all" ? "bg-ink text-background" : "border border-line bg-surface text-ink-3")}>Tümü</button>
            {tags.map((tag) => <button key={tag} type="button" onClick={() => setFilter(tag)} className={cn("rounded-full px-2.5 py-1 text-[0.6875rem] font-medium", filter === tag ? "bg-ink text-background" : "border border-line bg-surface text-ink-3")}>{tagLabels[tag]}</button>)}
          </div>
        ...
        <section className="order-1 flex h-[36rem] min-w-0 flex-col sm:h-[42rem] xl:order-2 xl:h-auto">
          <div className="flex items-center justify-between border-b border-line px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-ink">CareerForge Koç</span>
            </div>
            <span className="text-[0.6875rem] text-ink-3">{hasCv ? `${cvContext.title} bağlamı` : "Genel oturum"}</span>
          </div>

          <div className="flex-1 space-y-6 overflow-x-hidden overflow-y-auto px-4 py-6 sm:px-7">
            ...
            {loading && (
              <div className="flex items-center gap-2 text-xs text-ink-3"><Loader2 className="h-3.5 w-3.5 animate-spin" /> CV bağlamı değerlendiriliyor</div>
            )}
            <div ref={bottomRef} />
          </div>
          ...
          <form onSubmit={send} className="border-t border-line bg-surface-2 p-3 sm:p-4">
            <div className="flex items-end gap-2">
              <Textarea
                ...
                placeholder="Örn. Performans kazanımımı STAR formatında anlatmama yardım et"
                className="min-h-12 max-h-32 resize-none bg-surface"
                aria-label="Koça mesaj"
              />
              <Button type="submit" variant="primary" size="icon" disabled={!input.trim() || loading} aria-label="Mesajı gönder" title="Gönder"><Send className="h-4 w-4" /></Button>
            </div>
            <p className="mt-2 text-[0.625rem] text-ink-3">Enter ile gönder · Shift + Enter ile yeni satır</p>
          </form>
        </section>

        <aside className="order-3 border-t border-line p-5 xl:border-l xl:border-t-0">
          <div className="border-b border-line pb-5"><CvUploadPanel lang="tr" /></div>

          <div className="mt-6">
            <div className="flex items-center justify-between"><p className="section-label">CV sinyalleri</p><Sparkles className="h-4 w-4 text-info" /></div>
            <div className="mt-4 space-y-3">
              ...
              )) : <p className="text-xs leading-5 text-ink-3">Özgeçmiş eklendiğinde kişisel sinyaller burada görünür.</p>}
            </div>
          </div>

          {hasCv && (
            <dl className="mt-7 grid grid-cols-2 gap-px border border-line bg-line">
              <div className="bg-surface p-3"><dt className="text-[0.625rem] text-ink-3">Beceri</dt><dd className="metric-number mt-1 text-lg font-semibold text-ink">{cvContext.skillCount}</dd></div>
              <div className="bg-surface p-3"><dt className="text-[0.625rem] text-ink-3">Deneyim</dt><dd className="metric-number mt-1 text-lg font-semibold text-ink">{cvContext.expCount}</dd></div>
            </dl>
          )}
        </aside>
      </div>
