                ...
                {weeklyGoals.map((goal, i) => (
                  ...
                  >
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <span className="mt-0.5 block text-xs text-ink-3">/ {goal.target}{goal.unit}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-2.5 text-center sm:grid-cols-5">
                {[
                  ...
                  >
                    ...
                    <span className="mt-1.5 w-full truncate text-xs font-semibold uppercase tracking-wider">{item.label}</span>
                  </motion.div>
                ))}
              </div>
            </div>
                  ...
                  <li key={action.title}>
                    <Link href={action.href} className="interactive-row group grid grid-cols-[2rem_1fr_auto] items-center gap-4 border-b border-line py-5">
                      <span className="grid h-6 w-6 place-items-center rounded-full border border-line-strong text-ink-3">
                        {action.done ? <Check className="h-3.5 w-3.5 text-positive" /> : <span className="font-mono text-xs">0{index + 1}</span>}
                      </span>
                      <div>
          ...
          <aside className="space-y-8 xl:sticky xl:top-32 xl:self-start">
            ...
            <section className="bg-surface border border-line rounded-lg p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/resume" className="inline-flex min-h-11 items-center text-xs font-semibold text-brand-strong hover:underline">{isTr ? "Tümünü Gör" : "See All"}</Link>
              </div>
              <div className="space-y-3">
                {forgeBackups.slice(0, 3).map((backup) => (
                  <div key={backup.id} className="flex justify-between items-start text-xs border-b border-line pb-2.5 last:border-0 last:pb-0">
                    ...
                    <Link href="/resume">
                      <Button variant="ghost" size="sm" className="shrink-0 text-xs">{isTr ? "Gözat" : "View"}</Button>
                    </Link>
                  </div>
                ))}
                {!forgeBackups.length && (
                    ...
                    <Link key={job.id} href={`/jobs/${job.id}`} className="interactive-row block border-t border-line py-4 first:border-t-0 first:pt-0">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-ink-3">{company?.name} · {workModeLabels[job.workMode]}</p>
                        </div>
